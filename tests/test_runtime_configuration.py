...
def test_producao_resolve_segredos_de_arquivo_para_o_banco_do_container(
    ...
    control_token_path = tmp_path / "rtd_control_token"
    ...
    control_token_path.write_text("token-de-arquivo-com-tamanho-suficiente", encoding="utf-8")
    monkeypatch.delenv("SECRET_KEY", raising=False)
    monkeypatch.delenv("DATABASE_URL", raising=False)
    ...
    monkeypatch.setenv("RTD_CONTROL_TOKEN_FILE", str(control_token_path))

...
def test_delete_quote_history_entry(app: Flask, auth_client: FlaskClient) -> None:
    with app.app_context():
        ticker_id = _seed_ticker()
    ...
    with app.app_context():
        entry_id = db.session.query(QuoteHistory).filter_by(ticker_id=ticker_id).one().id

    response = auth_client.post(f"/quotes/{entry_id}/delete", follow_redirects=True)
    assert response.status_code == 200
    with app.app_context():
        assert db.session.get(QuoteHistory, entry_id) is None

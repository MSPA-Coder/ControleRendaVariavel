...
class PowerShellAutomationController:
    def __init__(self, project_dir: Path, script: Path | None = None) -> None:
        configured = os.getenv("RTD_AUTOMATION_SCRIPT", "scripts/rtd-automation.ps1")
    ...
    def set_enabled(self, enabled: bool) -> None:
        self._run("Enable" if enabled else "Disable")
        ...
        *,
        ...
        if self.available:
            try:
                ...
                if (
                    self._profile == OperationalProfile.PRODUCTION
                    and self._automation_status != "enabled"
                ):
                    self._automation.set_enabled(True)
                    self._automation_status = "enabled"
                elif (
                    self._profile == OperationalProfile.TEST
                    and self._automation_status == "enabled"
                ):
                    self._automation.set_enabled(False)
                    self._automation_status = "disabled"
        ...
        with self._lock:
            ...
            if profile == OperationalProfile.PRODUCTION:
                ...
                try:
                    ...
                    try:
                        self._profile_store.write(previous)
                        self._automation.set_enabled(False)
                        self._automation_status = "disabled"
                    except (OSError, RuntimeError):
                        self._automation_status = "error"
            ...
            try:
                ...
                try:
                    ...
                    self._automation_status = "error"
                self._desired_running = previous == OperationalProfile.PRODUCTION
                self._status = "error"

...
class TickerImportTarget:
    ...
    """Decide QUAL preço do Yahoo é gravado — ver ``fetch_yahoo_daily_quotes``.
    Um ticker de referência não é detido por ninguém, então nunca terá renda
    cadastrada contra ele; é o único caso em que o preço ajustado é o certo."""
...
def fetch_yahoo_daily_quotes(
    ...
    *,
    ...
    if not isinstance(timestamps, list) or not isinstance(indicators, dict):
        raise QuoteHistoryImportError(f"Invalid Yahoo history received for {target.symbol}.")
    # Ativo detido grava `quote.close` (nominal); ticker de REFERENCIA grava
    # `adjclose` (ajustado por provento). Os dois sao ajustados por
    # desdobramento -- verificado no split 10:1 da NVDA em 2024-06-10, em que
    # o `close` nao salta --, entao a escolha muda so o tratamento de
    # provento.
    #
    # A assimetria e deliberada e vem de quem PODE ter renda cadastrada. Para
    # um ativo da carteira, o `adjclose` reescreveria o passado a cada
    # provento e faria dividendo e JCP contarem DUAS vezes quando lancados,
    # alem de deixar o aluguel de acoes invisivel -- nenhuma fonte de preco o
    # embute. Para um ticker de referencia, nao existe cadastro possivel:
    # `investable_ticker_records` o mantem fora do formulario de rendas
    # justamente porque ele nao e detido. Gravar o nominal dele compararia a
    # carteira COM renda contra um indice SEM renda, o que subestimaria o
    # indice de graca.
